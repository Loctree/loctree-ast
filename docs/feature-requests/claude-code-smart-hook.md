...
| `dead` / `unused` | Find dead code | `loct '.dead_parrots' --artifact findings` is instant |
| `circular` | Find cycles | `loct '.cycles' --artifact findings` pre-indexed |
...
7. **dead/unused/orphan** → `loct '.dead_parrots' --artifact findings`
8. **circular/cycle** → `loct '.cycles' --artifact findings`
9. **duplicate/twin** → `loct twins --json`
...
(Created during Vista development session.)
