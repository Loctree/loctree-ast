...
# Set AICX_RECALL_DEDUP=0 to skip adjacent-line dedup.
...
# ── Stage 2: adjacent dedup ─────────────────────────────────────────────
# Collapses consecutive identical lines — handles repeated system-reminder
# blocks, status outputs, blank line spam. Cheap, safe, preserves order.
if [ "$DEDUP" = "1" ]; then
  uniq "${processed}.s1" >"$processed"
else
...
#
# P0 must be the last SUBSTANTIVE operator ask, not the last user-channel event:
# slash commands ("/compact"), local-command caveats, and task-notifications all
# arrive as user turns but carry no goal — a P0 of "/compact" is useless (field
# report 2026-07-15). Substantive turns land in last_user; every user turn also
# lands in last_user_any as the fallback when a session has no substantive ask.
last_user="${chunk_dir}/.last_user"
last_user_any="${chunk_dir}/.last_user_any"
last_asst="${chunk_dir}/.last_asst"
rm -f "$last_user" "$last_user_any" "$last_asst"
awk -v uf="$last_user" -v uaf="$last_user_any" -v af="$last_asst" '
  function substantive(b,   n, arr, i, line) {
    n = split(b, arr, "\n")
    for (i = 1; i <= n; i++) {
      line = arr[i]
      gsub(/^>[[:space:]]*/, "", line)
      ...
      if (line == "") continue
      if (line ~ /^\/[A-Za-z][A-Za-z0-9_:-]*([[:space:]]|$)/) return 0
      if (line ~ /^<(command-name|command-message|command-args|local-command-caveat|local-command-stdout|task-notification)/) return 0
      if (line ~ /^\[SYSTEM NOTIFICATION/) return 0
      return 1
    }
    return 0
  }
  function flush() {
    if (role=="u") {
      close(uaf); printf "%s", buf > uaf
      if (substantive(buf)) { close(uf); printf "%s", buf > uf }
    }
...
# Fallback: no substantive ask found anywhere → show the raw last user turn.
[ -s "$last_user" ] || last_user="$last_user_any"
...
# Window 800 + cap 12: a two-wave dispatch mentions 7+ commits spread over far more
# than 200 lines — the old 200/6 silently dropped fresh SHAs while keeping stale
# ones quoted late in the log (field report 2026-07-15). Bare 7-hex stays imprecise
# by design (git is the authority); the list is a recall hint, not a ledger.
shas=$(tail -n 800 "$processed" | grep -oiE 'commit[^0-9a-f]{0,4}[0-9a-f]{7,12}|\b[0-9a-f]{7}\b' \
  | grep -oE '[0-9a-f]{7,12}' | sort -u | head -n 12)
...
build_digest() {
  ...
  printf '   %s chunks (skill-stripped + deduped, %s%% smaller), chronological:\n' "$num_chunks" "$reduction_pct"
